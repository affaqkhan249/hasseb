...
const Navbar = () => {
  ...
  return (
    <header className="relative w-full z-30">
      {/* ---------- Top Header ---------- */}
      <div className="w-full bg-dark-navy py-3 md:py-0 md:h-[75px] flex items-center">
        <div className="w-full max-w-[1440px] mx-auto px-5 md:px-8 lg:px-12 flex flex-wrap items-center justify-between gap-3">
          {/* Logo */}
          <a href="/" aria-label="Elite Holidays Home" className="flex flex-col leading-none">
            <span className="flex items-center gap-1.5 font-poppins font-bold text-xl md:text-[26px] tracking-wide text-white">
              ...
              <FaPlane className="ml-1.5 text-sm md:text-base text-light-gold rotate-45" aria-hidden="true" />
            </span>
          ...
          <div className="flex items-center gap-3.5 md:gap-5">
            <div className="flex items-center gap-3">
              ...
              <span className="hidden sm:flex flex-col">
                ...
                <span className="text-sm font-semibold text-white">+44 20 4572 2672</span>
              </span>
            </div>

            <span className="hidden md:block w-px h-9 bg-white/20" aria-hidden="true" />
      ...
      {/* ---------- Floating Navigation ---------- */}
      <nav className="relative w-full flex justify-center -mt-px z-40" aria-label="Primary navigation">
        <div className="w-[calc(100%-32px)] md:w-[calc(100%-64px)] lg:w-[calc(100%-96px)] max-w-[1344px] bg-primary-navy rounded-2xl md:rounded-2xl shadow-nav flex flex-wrap items-center justify-between px-4 md:px-5 py-2.5 md:py-2.5 md:pl-7">
          {/* Mobile toggle */}
          <button
            ...
            {navLinks.map((link) => (
              <li key={link} className="w-full md:w-auto">
                <a
                  ...
                  className={`flex items-center gap-1.5 w-full md:w-auto whitespace-nowrap text-sm font-medium tracking-wide px-4 md:px-[18px] py-3 rounded-full transition-colors duration-300 ${
                    active === link
                ...
                >
                  {link}
                  {link === "Countries" && (
                    <FaChevronDown className="text-[10px] mt-0.5" aria-hidden="true" />
                  )}
                </a>
              </li>
            ))}
